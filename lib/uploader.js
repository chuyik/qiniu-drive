  ...
  return new Promise ((resolve, reject) => {
    qiniu.io.putFile(token, key, localFile, extra, (err, ret) => {
      if(!err) {
        ...
        console.log(`---- ${'Uploaded'.magenta}: `, `http://${domain}/${ret.key}?_=${Date.now()}`.underline.blue)
        resolve(ret)
        ...
        // 上传失败， 处理返回代码
        console.error(`---- ${'Upload error'.red}: `, err)
        reject(err)
      }
    })
  })
...
function removeFile(bucket, key) {
  return new Promise ((resolve, reject) => {
    client.remove(bucket, key, err => {
      if (!err) {
        console.log(`---- ${'Deleted'.magenta}: `, key)
        resolve()
      } else {
        console.error(`---- ${'Failed to delete'.red}: `, err)
        reject(err)
      }
    })
  })
  ...
  folders.forEach(folder => {
    ...
    walk(dir, data.watchOption)
      .then(files => {
        ...
        console.log(`-- ${'Sync: '.magenta}`, `${uploadCount} files uploading, ${ignoredCount} files passed.`.yellow)
        promises.push(Promise.map(filePaths, f => {
          let key = util.getBucketKey(folder, f)
